...
///可以控制版本超时时间的本地缓存策略，此类中key可以为任意key，内部文件会做MD5加密处理，应保证key的唯一性。
@interface DWFlashFlowLocalCache : NSObject<DWFlashFlowCacheProtocol>
...
/**
 ...
 @param cacheSize 最大磁盘缓存空间(单位B,即1KB为1024)
 @return 实例
 */
+(instancetype)cacheHandlerWithMaxExpireInterval:(NSTimeInterval)expireInterval maxCacheSize:(unsigned long long)cacheSize;

//Initialize with default value.ExpireInterval is seconds in one week,diskSize is 100MB.
///以默认值初始化，其中超时时间为1周，磁盘缓存为100MB。
+(instancetype)cacheHandler;

@end

//Cache response via NSURLCache.(Can also control expired time and AppVersion in Local cache.)
///通过NSURLCache进行内存及磁盘缓存，可以控制最大超时时间、缓存超时时间、版本号。具有自动清理磁盘缓存功能。
...
/**
 Specific initialize method.
 初始化方法

 @param expireInterval 超时时间
 @param memorySize 内存缓存限制
 @param diskSize 磁盘缓存限制
 @return 实例
 
 @disc expireInterval若为0则无最大超时时长
 */
...
///以默认值初始化，其中超时时间为1周，内存缓存限制为2MB，磁盘缓存为100MB。
+(instancetype)cacheHandler;
